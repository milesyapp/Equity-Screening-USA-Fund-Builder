...
def build_allocation(close: pd.DataFrame, market: dict) -> dict:
    ...
    # Where the ACTUAL (risk-parity) portfolio sits, in the SAME return/cov model
    # the frontier is drawn from — so the marker is honest. Risk parity is not a
    # return-optimized point, so it typically sits just inside the frontier.
    fmean, fcov = portfolio_optimizer._annualized_inputs(risky_daily)
    fmean = portfolio_optimizer._shrink_returns(fmean, settings.RETURN_SHRINKAGE)
    rp_vec = np.array([rp_w[k] for k in risky])
    portfolio_point = {
        "volatility": float(np.sqrt(rp_vec @ fcov.values @ rp_vec)),
        "return": float(rp_vec @ fmean.values),
    }
    ...
    return {
        ...
        "portfolioPoint": portfolio_point,
