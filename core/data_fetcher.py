...
def _with_retry(max_attempts: int = 3, backoff: float = 2.0):
    ...
    Retries on any exception. The final attempt re-raises so callers can still
    handle the failure. This keeps network blips from killing weekly runs.
    """
    def decorator(fn):
        @wraps(fn)
...
def _clean_ticker(t: str) -> str:
    # yfinance/Alpaca use '-' where S&P uses '.' (e.g. BRK.B -> BRK-B)
